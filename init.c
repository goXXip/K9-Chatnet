 ...
 * 
 * $Id: init.c 5 2004-03-29 01:29:50Z dane $ 
 *
 */
...
#undef LTSIZE
    ...
    if (s_MemoServ && (!user || stricmp(user, s_MemoServ) == 0))
#if defined(IRC_ULTIMATE) || defined(IRC_ULTIMATE3)
...
#endif
    if (s_HelpServ && (!user || stricmp(user, s_HelpServ) == 0))
#if defined(IRC_ULTIMATE) || defined(IRC_ULTIMATE3)
...
#endif
    if (s_OperServ && (!user || stricmp(user, s_OperServ) == 0))
#if defined(IRC_ULTIMATE) || defined(IRC_ULTIMATE3)
...
#endif
    if (s_GlobalNoticer && (!user || stricmp(user, s_GlobalNoticer) == 0))
#if defined(IRC_ULTIMATE) || defined(IRC_ULTIMATE3)
...
int init(int ac, char **av)
{
    ...
    if (debug || readonly || skeleton) {
        alog("K9 %s (compiled for %s) starting up (options:%s%s%s)",
             version_number, version_protocol,
             debug ? " debug" : "", readonly ? " readonly" : "",
             skeleton ? " skeleton" : "");
    } else {
        alog("K9 %s (compiled for %s) starting up",
             version_number, version_protocol);
    }
