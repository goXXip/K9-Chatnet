 ...
 *
 * $Id: messages.c 5 2004-03-29 01:29:50Z dane $
 *
 */
...
static int m_kill(char *source, int ac, char **av)
{
    ...
    if ((s_OperServ && stricmp(av[0], s_OperServ) == 0) ||
        ...
        (s_MemoServ && stricmp(av[0], s_MemoServ) == 0) ||
        ...
        (s_HelpServ && stricmp(av[0], s_HelpServ) == 0) ||
...
static int m_admin(char *source, int ac, char **av)
{
    send_cmd(ServerName, "372 %s :- Administrative info about Services.ChatNet.Org", source);
    send_cmd(ServerName, "372 %s :- ChatNet IRC Network IRC Services", source);
    send_cmd(ServerName, "372 %s :- Admin: Smith", source);
    send_cmd(ServerName, "372 %s :- E-Mail Address: services@chatnet.org", source);
}
...
static int m_privmsg(char *source, int ac, char **av)
{
    ...
    if (*av[0] == '#') {
        char *tok = strtok(av[1], " ");
        if (tok && (ci = cs_findchan(av[0])) && !stricmp(tok, s_ChanServ) && (tok = strtok(NULL, " "))) {
          char buf[BUFSIZE], *therest;
          therest = strtok(NULL, "");
          if((!stricmp(tok, "HELP") || (therest && *therest == '#')))
            snprintf(buf, sizeof(buf), "%s %s", tok ? tok : "", therest ? therest : "");
          else
            snprintf(buf, sizeof(buf), "%s %s %s", tok ? tok : "", av[0], therest ? therest : "");
          if (!is_oper(u) && CSOpersOnly)
            notice_lang(s_ChanServ, u, ACCESS_DENIED);
          else
            chanserv(u, buf,buf);     
        ...
        if (s_OperServ && ((stricmp(av[0], s_OperServ) == 0)
            ...
            if (!is_oper(u) && CSOpersOnly)
                ...
                chanserv(u, av[1],av[1]);
        } else if (s_MemoServ && ((stricmp(av[0], s_MemoServ) == 0)
                   || (s_MemoServAlias
...
/* Check whether the user is an operator */
    ...
    case 'u':{
            ...
            send_cmd(NULL, "242 %s :Services up %d day%s, %02d hours, %02d minutes, %02d seconds",
                     source, uptime / 86400,
                     (uptime / 86400 == 1) ? "" : "s",
...
{
    if (source)
        send_cmd(ServerName, "351 %s ChatNet IRC Services (Core Services) %s %s :%s -- %s",
...
int m_whois(char *source, int ac, char **av)
{
    ...
    if (source && ac >= 1) {
        ...
        else if (s_OperServ && stricmp(av[0], s_OperServ) == 0)
            clientdesc = desc_OperServ;
        else if (s_GlobalNoticer && stricmp(av[0], s_GlobalNoticer) == 0)
...
void moduleAddMsgs(void) {
    ...
    m = createMessage("ADMIN",     m_admin); addCoreMessage(IRCD,m);
