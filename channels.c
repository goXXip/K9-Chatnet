 ...
 * 
 * $Id: channels.c 5 2004-03-29 01:29:50Z dane $
 *
 */
...
//    send_cmd(s_ChanServ, "JOIN %s", av[1]);
//    alog("debug: JOIN %s", av[1]);
...
uaccess = NULL;
    ...
    if (get_ignore(user->nick) == NULL) {
		...
		if(c->ci)
		{
		for (access = c->ci->access, i = 0; i < c->ci->accesscount;
         access++, i++) {
       	 if (access->nc == nc) {
					uaccess = &c->ci->access[i];
			}
		}
    }
	}
		/**pedro*/	
		if(uaccess && uaccess->auto_option && (user->na->status & NS_IDENTIFIED))
		{
		
		if(stricmp(uaccess->auto_option, "T") == 0)
		{
			
			send_cmd(whosends(c->ci), "MODE %s +o %s", c->ci->name, user->nick);
		}
		else if(stricmp(uaccess->auto_option, "V") == 0)
		{
			send_cmd(whosends(c->ci), "MODE %s +v %s", c->ci->name, user->nick);
		}
		}
		...
		if (c->ci && c->ci->entry_message2)
            notice_user(whosends(c->ci), user, "%s", c->ci->entry_message2);
			
		if (c->ci && c->ci->entry_message3)
            notice_user(whosends(c->ci), user, "%s", c->ci->entry_message3);
		
		if (c->ci && c->ci->entry_message4)
            notice_user(whosends(c->ci), user, "%s", c->ci->entry_message4);
		
		if (c->ci && c->ci->entry_message5)
            notice_user(whosends(c->ci), user, "%s", c->ci->entry_message5);
			
		if (c->ci && uaccess && uaccess->comment && (user->na->status & NS_IDENTIFIED))
			  send_cmd("K9", "PRIVMSG %s :%s", c->ci->name, uaccess->comment);
        ...
        if (s_BotServ && c->ci && c->ci->bi) {
            ...
            if (c->usercount >= BSMinUsers && (c->ci->botflags & BS_GREET)
                ...
                send_cmd(c->ci->bi->nick, "PRIVMSG %s :SAY %s", c->name,
